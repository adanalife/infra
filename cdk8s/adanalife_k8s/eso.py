...
appear in the legacy manifests, all covered here:
  ...
  1. `dataFrom.extract`  — pull every top-level key of a JSON parameter value into the
     Secret (grafana OTLP, sentry, postgres). `extract=<parameter name>`.
  2. per-key `remoteRef` + `property` — map specific parameter JSON properties to named
...
class ESData:
    ...
    key: str  # SSM parameter name (fully qualified, /k8s/...)
...
def secret_store(
    ...
    """Per-namespace `aws-parameterstore` SecretStore. Byte-identical across
    envs (the in-namespace eso-aws-credentials routes to the right account).
    The construct id stays "secret-store" so existing rendered objects keep
    their identity across the SM → SSM migration."""
