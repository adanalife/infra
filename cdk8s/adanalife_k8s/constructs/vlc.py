...
class VlcServer(Construct):
    def __init__(self, scope: Construct, *, env: EnvConfig):
        ...
        if env.dashcam_mode == "nfs":
            ...
            volume = k8s.Volume(
                name="dashcam",
                host_path=k8s.HostPathVolumeSource(path="/host/dashcam", type="Directory"))

        # --- resources (+ iGPU on GPU envs) ---
        requests = {"cpu": k8s.Quantity.from_string("200m"),
        ...
        container = k8s.Container(
            ...
            ports=[k8s.ContainerPort(name=n, container_port=p) for n, p in container_ports],
            env_from=[
            ...
            volume_mounts=[k8s.VolumeMount(name="dashcam", mount_path=MOUNT_PATH, read_only=True)],
        )
        ...
        # --- Service ---
        svc_ports = [k8s.ServicePort(name=n, port=p, target_port=k8s.IntOrString.from_string(n))
        ...
        if appconfig.uses_local_stubs(env):
            k8s.KubeService(self, "host-access",
                ...
                spec=k8s.ServiceSpec(
                    type="LoadBalancer", selector=sel,
                    ports=[k8s.ServicePort(name="vnc", port=5903, target_port=k8s.IntOrString.from_string("vnc")),
                           k8s.ServicePort(name="rtsp", port=8554, target_port=k8s.IntOrString.from_string("rtsp"))]))
    ...
    def _ingress(self, env: EnvConfig, ns):
        ...
        k8s.KubeIngress(self, "ingress",
            ...
            spec=k8s.IngressSpec(
                ...
                tls=[k8s.IngressTls(hosts=[host], secret_name="vlc-tls")] if env.tls else None,
