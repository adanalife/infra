...
Emits, for one platform, into the env's app namespace:
  ...
  * Deployment mediamtx-{platform} — at most one replica, Recreate (a relay
    ...
    Stage births parked at replicas:0, activated by a console scale-up.
    ...
    def _instance(self, env: EnvConfig, platform: str):
        ...
        k8s.KubeDeployment(
            ...
            spec=k8s.DeploymentSpec(
                # Stage relays are declared parked: a console/hand scale-up is
                # what activates one, and Argo ignores .spec.replicas so the
                # scale sticks. Prod declares its relay live — it carries the
                # broadcast.
                replicas=1 if env.name == "prod-1" else 0,
