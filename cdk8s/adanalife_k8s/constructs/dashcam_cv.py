...
class DashcamCV(Construct):
    def __init__(self, scope: Construct, *, env: EnvConfig):
        ...
        self._image = f"{IMAGE}:{env.image_tag}"   # adanalife/dashcam-cv:develop on stage
        ...
        k8s.KubePriorityClass(self, "priorityclass",
        ...
        k8s.KubePersistentVolumeClaim(self, "models-pvc",
            metadata=k8s.ObjectMeta(name="dashcam-cv-models", namespace=ns, labels=labels),
        ...
        k8s.KubeCronJob(self, "fill-cronjob",
            metadata=k8s.ObjectMeta(name="dashcam-cv-fill", namespace=ns, labels=labels),
            spec=k8s.CronJobSpec(
                ...
                job_template=k8s.JobTemplateSpec(
                    spec=k8s.JobSpec(
                        ...
                        template=self._embed_pod_spec(labels, args=["embed", "--random", "3", "--interval", "5", "--apply"]),
                    ))))

        # --- Job: one-shot manual ramp (same pod, --random 1) ---
        k8s.KubeJob(self, "fill-job",
            metadata=k8s.ObjectMeta(name="dashcam-cv-fill-once", namespace=ns, labels=labels),
            spec=k8s.JobSpec(
                backoff_limit=0,
                ttl_seconds_after_finished=3600,
                template=self._embed_pod_spec(labels, args=["embed", "--random", "1", "--interval", "5", "--apply"]),
            ))
        ...
        return k8s.PodTemplateSpec(
            spec=k8s.PodSpec(
                ...
                containers=[k8s.Container(
                    ...
                    args=args,
                    env_from=[
                        k8s.EnvFromSource(config_map_ref=k8s.ConfigMapEnvSource(name="tripbot-config")),
                        k8s.EnvFromSource(secret_ref=k8s.SecretEnvSource(name="tripbot-database-creds")),
                    ],
                    env=[k8s.EnvVar(name="DASHCAM_CV_CORPUS_DIR", value=CORPUS_DIR)],
                    resources=k8s.ResourceRequirements(
                        requests={"cpu": k8s.Quantity.from_string("1"),
                                  "memory": k8s.Quantity.from_string("5Gi")},
                        limits={"cpu": k8s.Quantity.from_string("4"),    # hard CPU cap so prod keeps its cores
                ...
                volumes=[
                    k8s.Volume(name="dashcam",
                               persistent_volume_claim=k8s.PersistentVolumeClaimVolumeSource(
                                   claim_name="vlc-dashcam")),   # stage NFS corpus, ro
                    k8s.Volume(name="models",
                               persistent_volume_claim=k8s.PersistentVolumeClaimVolumeSource(
