...
class EnvConfig:
    ...
    deployment_env: str = "development"  # OTEL deployment.environment + telemetry env id
    ...
    external_dns_role_arn: str = ""     # cert-manager DNS-01 Route53 role (per AWS account)
    lan_ip: str = "192.168.1.200"       # mini-PC node IP external-dns/traefik target (platform Helm)
    nfs_server: str = ""                # dashcam NFS export (nfs mode); from $NFS_SERVER at synth
    nfs_path: str = ""                  # dashcam NFS path; from $NFS_PATH at synth
    nfs_pv_name: str = "vlc-dashcam-nfs"  # PVs bind 1:1 — stage needs its own (vlc-dashcam-nfs-stage)
    vlc_inpod_onscreens: bool = False   # prod-only: re-expose the in-pod onscreens on :8081 until OBS cutover
...
ENVS: dict[str, EnvConfig] = {
    "prod-1": EnvConfig(
        ...
        sentry_env="prod-1", binary_env="production", deployment_env="prod-1",
        gpu=True, obs_encoder="ffmpeg_vaapi_tex",
        obs_quality="high", dashcam_mode="nfs", tailscale=True, otel=True,
        postgres_size="50Gi", postgres_storage_class="local-path-retain",
        postgres_backup=True, external_dns_role_arn=_PROD_ROLE,
        nfs_pv_name="vlc-dashcam-nfs", vlc_inpod_onscreens=True,
        platforms=("twitch",),
    ),
    "stage-1": EnvConfig(
        ...
        sentry_env="stage-1", binary_env="staging", deployment_env="stage-1",
        gpu=True, obs_encoder="ffmpeg_vaapi_tex",
        obs_quality="low", dashcam_mode="nfs", tailscale=True, otel=False,
        postgres_size="10Gi", postgres_storage_class="local-path",
        external_dns_role_arn=_STAGE_ROLE, nfs_pv_name="vlc-dashcam-nfs-stage",
    ...
    "development": EnvConfig(
        ...
        sentry_env="development", binary_env="staging", deployment_env="development",
        gpu=False, obs_quality="low",
        dashcam_mode="hostpath", tailscale=False, otel=False,
        external_dns_role_arn=_STAGE_ROLE, platforms=("twitch",),
    ),
    "local": EnvConfig(
        name="local", namespace="default", cluster="local",
        aws_account="", image_tag="latest", dns_base="",
        nats_url="", sentry_env="development", binary_env="staging",
        deployment_env="development", secret_source="local",
