...
# is the (PGP-encrypted) access key. `task k8s:<env>:bootstrap-secrets`
...
resource "aws_iam_policy" "allow_eso_read_k8s_secrets" {
  ...
  description = "Read-only access for in-cluster ESO to k8s/* SSM parameters in ${var.account_name}."
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      # rides the AWS-managed aws/ssm KMS key — no explicit kms grant needed.
      {
        ...
        Resource = [
          "arn:aws:ssm:${data.aws_region.current.region}:${data.aws_caller_identity.current.account_id}:parameter/k8s/*",
        ]
      },
    ]
  })
