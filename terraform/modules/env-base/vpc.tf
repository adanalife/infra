...
# (expected: module.env_base.module.default_vpc.aws_default_vpc.this[0]).
# Adjust the `from` below if your state shows a different internal address,
# then `terraform plan` and confirm it reports a move with NO destroy/create
# before applying.
moved {
