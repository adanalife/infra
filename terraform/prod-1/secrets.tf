...
#
# Prod-only mirrors live next to their SM originals: argocd.tf (repo deploy
# keys), tailscale.tf (api key + operator oauth + node authkey),
# postgres-backup.tf (backup S3 credentials) — same KEEP-IN-SYNC reasoning
# as the SM containers themselves.
