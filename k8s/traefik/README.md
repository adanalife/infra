...
In-cluster ingress controller, installed via `traefik/traefik` helm chart by `task k8s:dev:platform:up`. Replaces the k3s-bundled Traefik (disabled in `k8s/k3d-config.yaml` via `--disable=traefik`).

App Ingresses opt in via `ingressClassName: traefik`. The release also registers itself as the default IngressClass for any Ingress that doesn't specify one.
