...
In-cluster ingress controller, installed via `traefik/traefik` helm chart by `task k8s:dev:platform:up`. Replaces the k3s-bundled Traefik (disabled in `k8s/k3d-config.bees.yaml` via `--disable=traefik`).
